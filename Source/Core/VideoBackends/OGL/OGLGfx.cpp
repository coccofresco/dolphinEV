...
void OGLGfx::SetFramebuffer(AbstractFramebuffer* framebuffer)
{
  ...
  if (g_ActiveConfig.bVRImmersiveMode && (framebuffer->GetWidth() % 640 > 0) && (framebuffer->GetWidth() < 1000))
  {
...
void OGLGfx::RestoreFramebufferBinding()
{
  glBindFramebuffer(
    ...
    if (m_current_framebuffer && g_ActiveConfig.bVRImmersiveMode && (m_current_framebuffer->GetWidth() % 640 > 0) && (m_current_framebuffer->GetWidth() < 1000))
    {
        glBindFramebuffer(GL_FRAMEBUFFER, -1);
    }
}
